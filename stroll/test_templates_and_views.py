...
class ProjectViewsAndUrlsTests(TestCase):
    ...
    def setUp(self):
        ...
        self.client.force_login(user_profile.user)

        thumbnail_path = os.path.join(settings.MEDIA_ROOT, 'population_thumbnails/govan.jpg')
        with open(thumbnail_path, 'rb') as f:
            Walk.objects.create(user=user, title='Govan walk', description='Test walk', tags='test,tag', thumbnail=File(f, name='govan.jpg'))

        Question.objects.create(user=user_profile, question='Test question')
    ...
    def view_and_mapping_exist_using_correct_template(self, view_name, slug=''):
        ...
        if view_name == 'show_walk':
            url = reverse(f'stroll:{view_name}', kwargs={'id': '1'})

        elif view_name == 'show_question':
            url = reverse(f'stroll:{view_name}', kwargs={'id': '1'})

        elif view_name == 'user_login':
            view_name = 'login'
            url = reverse('stroll:login')
        ...
        if view_name == 'login':
            match = resolve(url)
            self.assertEquals(match.func, getattr(self.views_module, 'user_login'), f'{STANDARD_FAILURE} The view "{view_name}" is not mapped correctly')
        ...
        self.assertTemplateUsed(response, template_path, f'{STANDARD_FAILURE} The view "{view_name}" does not use a template or a correct template {response.status_code}')
    ...
    def test_login_view_exisits(self):
        ProjectViewsAndUrlsTests.view_and_mapping_exist_using_correct_template(self, 'user_login')
